...
Springboot中可以用@validated来校验数据，如果数据异常则会统一抛出异常，方便异常中心统一处理。比如：
...
利用配置文件到属性值的映射，spring可以通过大量的properties.class配置类（如DataSourceProperties.class），自动装配默认属性，进而被更多的地方引用
